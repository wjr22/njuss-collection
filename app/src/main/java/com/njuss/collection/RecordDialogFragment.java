...
public class RecordDialogFragment extends BaseDialogFragment {
    ...
    private void readyRecord() {//
        // TODO 删除原有文件---记得改文件名为  LicenseID
        final Store store;
        Bundle arguments = getArguments();
        store = (Store)arguments.getSerializable("store");
        App.getInstance().deleteFile( store.getLicenseID() + ".mp3");
       // FileUtils.deleteFile(FileUtils.getFilePath());//filename   wjr
        setClickable(false);
        record();
    }
...
//            Toast.makeText(getActivity(), "文件保存成功,路径是" + fileUri, Toast.LENGTH_SHORT).show();
    ...
    @OnClick({R.id.ll_cancel,R.id.ll_return, R.id.ll_send})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.ll_cancel:
                onCancelInterface.onCancel();
                FileUtils.deleteFile(FileUtils.getCacheFilePath(fileName));//删除
                LightenScreen();
                dismiss();
                break;
            case R.id.ll_send:
...
//        notifyServerFails("录音发送失败！");
    ...
    private void notifyServerSuccess() {
        ...
        changeGray();
        renderCardView("您的录音已经采集成功",R.mipmap.broadcast_success);
